...
func CreateInterior()
{
  ...
  CreateObject(LADR, 4680, 538, -1)->Set(8, LADR_Left|LADR_Right|LADR_Front|LADR_Additional, "W");
...
public func ChooserFinished()
{
  ...
  if(FindObject(GOCC))
  {
   ...
   SetupVehicleSpawn([PBOT],DIR_Left,CreateObject(VSPW,2195,509,-1),50*21,300);
  ...
  if (FindObject(GASS))
  {
   ...
   //MAV-Station
   CreateObject(MVSN, 7690, 320, -1)->Set(7620,320,1);

   //Patrouillenboote
   SetupVehicleSpawn([PBOT],DIR_Right,CreateObject(VSPW,1245,510,-1),50*21,300);
   SetupVehicleSpawn([PBOT],DIR_Left,CreateObject(VSPW,2195,509,-1),50*21,300);
   SetupVehicleSpawn([PBOT],DIR_Right,CreateObject(VSPW,4640,549,-1),50*21,300);
   SetupVehicleSpawn([PBOT],DIR_Right,CreateObject(VSPW,6945,509,-1),50*21,200);
  }
...
public func RelaunchPosition(& iX, & iY, int iTeam)
{
  ...
  if(FindObject(GCTF))
  {
   if(iTeam == 1)
   {
    return [[3600, 400], [3675, 320]];
   }
   if(iTeam == 2)
   {
    return [[6325, 500], [6460, 500]];
   }
   return(1);
  }
