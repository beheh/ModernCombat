...
protected func GuardAI_Off() { return guard_ai == 0; }
protected func GuardAI_On() { return guard_ai == 1; }
protected func GuardAI_Auto() { return guard_ai == 2; }

protected func ContextGuardAI_Off(object pCaller)
{
	[$CtxGuardAI$|Image=FLNT|Condition=GuardAI_Auto]
	GuardAI_Menu();
}

protected func ContextGuardAI_On(object pCaller)
{
	[$CtxGuardAI$|Image=NONE|Condition=GuardAI_Off]
	GuardAI_Menu();
}

protected func ContextGuardAI_Auto(object pCaller)
{
	[$CtxGuardAI$|Image=ROCK|Condition=GuardAI_On]
	GuardAI_Menu();
}

protected func GuardAI_Menu()
{
	var curIcon;
	if(guard_ai == 0) curIcon = NONE;
	else if(guard_ai == 1) curIcon = ROCK;
	else if(guard_ai == 2) curIcon = FLNT;
	
	CreateMenu(curIcon, this, this, 0, 0, 0, C4MN_Style_Context, false, 2HUD); 
	AddMenuItem("$CtxGuardAIOn$", "SetGuardAI", ROCK, this, 0, 1, "$CtxGuardAIOnDesc$"); 
	AddMenuItem("$CtxGuardAIOff$", "SetGuardAI", NONE, this, 0, 0, "$CtxGuardAIOffDesc$"); 
	AddMenuItem("$CtxGuardAIAuto$", "SetGuardAI", FLNT, this, 0, 2, "$CtxGuardAIAutoDesc$"); 
	SelectMenuItem(1, this);
	
	SetGuardAI(NONE, 0); // Erstmal ausschalten.
}

protected func OnMenuSelection(int iItemIndex, object pMenuObject)
{
	if((pMenuObject != this) && (GetMenu() != 2HUD))
		return _inherited(iItemIndex, pMenuObject, ...);
	
	if(iItemIndex == 1) // Beim mittlerem  Item zählt das nicht. ^^
		return;
	
	SetGuardAI(NONE, iItemIndex);
	CloseMenu(this);
}

protected func SetGuardAI(id idUnused, int iMode)
{
  Sound("Click", 1, 0,0, GetOwner()+1);
	guard_ai = iMode;
	var effect = GetEffect("IntGuardAI", this);

	if(iMode != 0)
	{
		if(!effect)
			AddEffect("IntGuardAI", this, 1, GUARDAI_PACE, this, GetID(), iMode-1);
		else
			EffectCall(this, effect, "Mode", iMode-1);
	}
	else
	{
...
{
	EffectCall(pTarget, iEffectNumber, "Mode");
}

public func FxIntGuardAIMode(object pTarget, int iEffectNumber)
{
	// ...
}

protected func GuardAI_GetAngle()
{
  if(!crosshair)
  {
    if(GetDir() == DIR_Left)
...
protected func GuardAI_GetWeaponScore(object & pEnemy, object & pWeapon, int & iFM, bool & bMayAim)
{
	var dst = ObjectDistance(this, pEnemy);
	var score = 0;

	if(dst > pWeapon->GetFMData(BOT_Range, i))
		score -= (dst - pWeapon->GetFMData(BOT_Range, i)) / 10;
	
	if(pWeapon->GetFMData(BOT_Ballistic, i))
		score -= 5;
	
	var power = pWeapon->GetFMData(BOT_Power, i);
	if(power == BOT_Power_LongLoad)
		score -= 4;
	else
		score += (power-1)*2;
	
	if(!bMayAim && (pWeapon->GetFMData(FM_Aim, i)))
		score -= 5;
...
protected func GuardAI_SelectWeapon(object & pEnemy, bool & bMayAim)
{
	var best_wpn = 0;
	var best_fm = 0;
	var best_score = 0;
	...
	for(var wpn in FindObjects(Find_Container(this), Find_Func("IsWeapon")))
	{
		for(var fm = 0; GetFMData(FM_Name, fm); fm++)
		{
			cur_score = GuardAI_GetWeaponScore(pEnemy, wpn, fm, bMayAim);
	...
	if(best_score)
	{
		ShiftContents(this, false, GetID(best_wpn), true);
		best_wpn->SetFireMode(best_fm);
		// An dieser Stelle nicht nachladen.
		return true;
	}
...
public func FxIntGuardAIIdle(object pTarget, int iEffectNumber, iEffectTime)
{
	// ... Waffen nachladen und so...
}

public func FxIntGuardAITimer(object pTarget, int iEffectNumber, int iEffectTime)
{
	var enemys = FindTargets(this, GUARDAI_RANGE, GUARDAI_MAXANGLE, GuardAI_GetAngle(), false);

	if(!GetLength(enemys)) // Keine Feinde (mehr)?
	{
		EffectCall(pTarget, iEffectNumber, "Idle", iEffectTime);
	}
	else
	{
		var mayaim = (guard_ai == 1) || !GetCommand();
		if(!GuardAI_SelectWeapon(enemys[0], mayaim))
		{
			// Wtf? Wir sind ihnen ausgeliefert?
			// Unseren Spieler benachrichtigen!
			
			var i, selected = false;
			while(GetCursor(GetOwner(pTarget), i++) != pTarget)
			for(var i = 0; ; i++)
			{
				GetCursor(GetOwner(pTarget), i)
			}
		}
		else
		{
...
public func FxIntGuardAIStop(object pTarget, int iEffectNumber, int iReason, bool fTemp)
{
