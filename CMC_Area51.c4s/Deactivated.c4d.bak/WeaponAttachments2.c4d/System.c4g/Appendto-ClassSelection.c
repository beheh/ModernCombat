...
func InitClassMenu(object pClonk)
{
  ...
  //Bereits ein Men¸ vorhanden: Schlieﬂen
  if(GetMenu(pClonk))
    CloseMenu(pClonk);
...
public func OpenMenuAttachment(id idParamWeapon, int iClass, object pClonk, int iSelection)
{
  ...
  {
    //Bei aktivierter Keine Waffenaufs‰tze-Regel Klasseninformationen anzeigen
...
public func OnGameOver()
{
  ...
  //-> Wird jedoch nur zu Rundenende(!) aufgerufen, daher keine groﬂartigen Spielverluste.
