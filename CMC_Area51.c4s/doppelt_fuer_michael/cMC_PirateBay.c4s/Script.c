...
public func ChooserFinished()
{
  ...
  if(FindObject(GDAR))
  {
   SetupComputerSpawnpoint([[4110, 570], [4140, 760]]);
   ...
   //Sendeanlagen platzieren
   CreateObject(TRMR, 4005, 525, -1)->Set("$Transmitter1$");
   CreateObject(TRMR, 3475, 890, -1)->Set("$Transmitter2$");
   CreateObject(TRMR, 4615, 920, -1)->Set("$Transmitter3$");
  }
...
public func RelaunchPosition(& iX, & iY, int iTeam)
{
  ...
  if(FindObject(GHTF) || FindObject(GDAR) || FindObject(GCTF) || FindObject(GLMS) || FindObject(GTDM))
  {
   if(iTeam == 1)
   {
