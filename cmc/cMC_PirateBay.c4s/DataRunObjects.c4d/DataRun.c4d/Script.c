...
public func PlaceComputerSpawnpoint(int iX, int iY, bool fNoDelay, bool fRespawnCooldown)
{
  if(!iX && !iY)
  {
  ...
  else if(fRespawnCooldown) {
  	var e = GetEffect("CompEffect", comp);
  	
  	//Respawn-Timer setzen
  	if(e) {
  		EffectVar(0, comp, e) = true;
  		EffectVar(1, comp, e) = GDAR_NoSignalTimer;
  	}
  }
...
public func OnDeathAnnounce(object pCrew, int iKiller, int iAssist)
{
  ...
  if(!(iKiller == NO_OWNER || iPlr == iKiller || GetPlayerTeam(iPlr) == GetPlayerTeam(iKiller)))
  {
    ...
    var data_dead = BoundBy(aData[index], 2, 10);
    if(data_dead)
    	//Wird subtrahiert, damit der groeßere Anteil in den Datentraeger landet (sonst geht 1 Punkt wegen der Rundung verloren)
      CreateObject(DASE, GetX(pCrew), GetY(pCrew), GetOwner(pCrew))->Set(data_dead-data_dead/2); 

    GiveData(iPlr, Max(-aData[index], -data_dead));
    GiveData(iKiller, data_dead/2);
  }
  else if(iPlr == iKiller || GetPlayerTeam(iPlr) == GetPlayerTeam(iKiller)) {
  	var index = iPlr;
  	if(Teams())
  		index = GetPlayerTeam(iPlr);
  	
  	var data_dead = BoundBy(aData[index], 0, 5);
  	if(data_dead)
      CreateObject(DASE, GetX(pCrew), GetY(pCrew), GetOwner(pCrew))->Set(data_dead);
    
    GiveData(iPlr, -data_dead);
  }
...
private func GiveData(int iPlr, int iAmount)
{
  ...
  //Punktevergabe
  if(iAmount > 0)
  	DoPlayerPoints(iAmount, RWDS_TeamPoints, iPlr, pClonk, IC20);
