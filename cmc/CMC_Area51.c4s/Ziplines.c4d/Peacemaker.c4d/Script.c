...
public func FxRideZiplineStart(object pTarget, int iNr, int iTemp, object pRope, int iRopePos)
{
	...
	Sound("GrenadeActivate.ogg", 0, pTarget, 40);
	Sound("LADR_Slide.ogg", 0, pTarget, 5, 0, +1);
	
	//Ggf. Fallschirm schließen
...
{
	SetXDir(EffectVar(4, pTarget, iNr)*2/3, pTarget, 100);
	SetYDir(EffectVar(5, pTarget, iNr)*2/3, pTarget, 100);
