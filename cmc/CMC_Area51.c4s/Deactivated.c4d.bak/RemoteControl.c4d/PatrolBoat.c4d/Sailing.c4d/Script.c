...
private func Sail()
{
  if(!FindObject(0,0,0,0,0,0,"Push",motor) && (!LocalN("pRemoteControl", motor) || !LocalN("pRemoteControl", motor)->IsControlling()))
    Stop();
...
//Der Owner darf nicht entfernt werden, wenn es ferngesteuert wird
func FxVehicleNoOwnerTimer(object pTarget, int iEffectNumber, int iTime)
{
	if(motor && LocalN("pRemoteControl", motor) && LocalN("pRemoteControl", motor)->IsControlling())
	{
...
//Das Fahrzeug darf nicht zerstört werden, wenn es ferngesteuert wird
func FxIntVehicleUnusedTimer(object pTarget, int iEffectNumber, int iTime)
{
