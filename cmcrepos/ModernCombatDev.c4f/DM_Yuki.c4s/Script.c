...
func CreateFurniture()
{
  ...
  CreateObject(LADR, 1725, 550, -1)->Set(20);
  ...
  //Gasflaschen
  CreateObject(GSBO, 585, 610, -1)->AutoRespawn();
  CreateObject(GSBO, 1000, 580, -1)->AutoRespawn();
  CreateObject(GSBO, 1500, 660, -1);
...
func CreateEquipment()
{
  ...
  var tmp = CreateObject (ABOX, 435, 660, -1);
  tmp->Set(AMOC);
  tmp->SetGraphics("Normal");
  var tmp = CreateObject (ABOX, 1000, 430, -1);
  tmp->Set(AMOC);
  tmp->SetGraphics("Normal");
  var tmp = CreateObject (ABOX, 1475, 660, -1);
  tmp->Set(AMOC);
  tmp->SetGraphics("Normal");

  //Raketen
  PlaceSpawnpoint(MIAP, 710, 510);
  ...
  //PzF
  PlaceSpawnpoint(PZFW, 700, 510);

  //MP7
  PlaceSpawnpoint(MP7R, 320, 390);
  PlaceSpawnpoint(MP7R, 1725, 610);

  //Pumpgun
  PlaceSpawnpoint(PPGN, 860, 750);

  //P29
  PlaceSpawnpoint(P29W, 1000, 740);

  //M249
  PlaceSpawnpoint(M249, 1520, 230);

  //Desert Eagle
  PlaceSpawnpoint(DEGL, 1190, 815);
