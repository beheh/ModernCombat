...
global func DoExplosion(int x, int y, int level, object inobj, int cause_plr, id effect_def, string effect_particle, object layer)
{
  ...
  if (!container)
  {
    ...
    {
      var vis;
      if(EffectLevel() > EM4K_Normal)//Bubbelblubberblasenspaß!
        if(GBackLiquid(x,y))
        {
          vis = 100;
          
          var k = 3;//Fragt mich nicht warum ich das K nenne. xD
          if(EffectLevel() > EM4K_High) k = 2;
