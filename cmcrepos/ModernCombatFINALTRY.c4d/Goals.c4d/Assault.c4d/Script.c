...
public func CreateTeamBorder(int iX, int iY, int iDir, bool fCreateBorderObject, int iTime)
{
  if(iTime <= 0) iTime = 30;
  iTime *= 35;
  ...
  if(fCreateBorderObject)
    for(var i = 0; i < GetPlayerCount(); i++)
      if(GetPlayerTeam(GetPlayerByIndex(i)) == iAttacker)
        ...
        EventInfo4K(GetPlayerByIndex(i) + 1, "$MsgRegroup$", SM03, 0, 0, 0);
      else
        //Eventnachricht: Zurückfallen
        EventInfo4K(GetPlayerByIndex(i) + 1, "$MsgFallBack$", SM03, 0, 0, 0);
...
public func FxTeamBorderTimer(object pTarget, int iEffectNumber, int iEffectTime)
{
  for(var i = 0; i < GetPlayerCount(); i++)
    if(GetPlayerTeam(GetPlayerByIndex(i)) == iAttacker)
      ...
      EventInfo4K(GetPlayerByIndex(i) + 1, "$MsgForward$", SM03, 0, 0, 0);
    else
      //Eventnachricht: Verteidigen
      EventInfo4K(GetPlayerByIndex(i) + 1, "$MsgDefend$", SM03, 0, 0, 0);

  var border = EffectVar(0, this, iEffectNumber);
  if(border != 0) RemoveObject(border);
  
  RemoveEffect("TeamBorder", this);
}
