...
func FxLaserDotTimer(object pTarget, int iEffectNumber, int iEffectTime)
{
  ...
  if(iAttachment == AT_Laserpointer)
  {
    ...
    var pEnemy;
    pEnemy = FindObject2(Find_OnLine(0, 0, x - xPos, y - yPos), Find_NoContainer(), Find_Or(Find_Func("IsSmoking"), (Find_Hostile(GetOwner(GetUser(this))), Find_Or(Find_OCF(OCF_Alive), Find_Func("IsBulletTarget", GetID(), this, this), Find_Func("IsCMCVehicle")))), Sort_Distance(0, 0));
