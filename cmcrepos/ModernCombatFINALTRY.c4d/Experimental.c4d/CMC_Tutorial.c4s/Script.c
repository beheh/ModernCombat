...
func StartPetrolDialog(object pClonk, object pPartner)
{
  if(!AIDialog(pClonk)->StartChat(1,pPartner))
  {
    ...
    AIDialog(pClonk)->StartDialog(pPartner);
    AIDialog(pClonk)->AddAnswer(0, "Entschuldigung","UnGrab",NONE);
  }
}
...
{
  CreateContents(ASTR,pClonk)->SetFireTec(1,1,true);//Einzelschuss.
  if(!FindObject2(Find_ID(RTLR), Find_AnyContainer())) {
    CreateContents(RTLR,pClonk);
  }
  else {
  
  }
  
  pClonk->DoAmmo(STAM,60);
  pClonk->DoAmmo(MIAM,10);
...
func AI_Marksman_StartDialog(object pClonk, int iEffect, object pPartner)
{
  ...
  if(!--rand)
    AIDialog(pClonk)->SetGreeting("Hau ab, ich muss mich konzentrieren.");
  if(!--rand)
...
func AI_Marksman_Timer(object pClonk, int iEffect)
{
  ...
  else
  {
    if(Random(3)) return();
    //Drauf knallen.
    pClonk->DoAmmo(pClonk->Contents()->GetFMData(FM_AmmoID), +1);
    if(WeaponReady(pClonk))
    {
      AimAtTarget(pClonk,aTargets[0]);
      Fire(pClonk);
      pWeapon->DoSpread(-pWeapon->GetSpread());
      if(!Random(10)) ShiftContents(pClonk, Random(1));
    }
    else {
      pClonk->DoAmmo(pClonk->Contents()->GetFMData(FM_AmmoID), -1);
    }
...
func Fire(object pClonk)
{
  ...
  else
  {
    ...
    if(!CheckAmmo(ammoid,ammousage,weapon))//nicht mehr genügend muni in der waffe?
    {
      ...
      {
        weapon->Reload();
        if(!weapon->GetFMData(FM_Aim)) pClonk->StopAiming();//xD Sieht halt toller aus. :)
      }
    }
  }
