/* Killverfolgung durch ShakeFree */

#strict 2

global func ShakeFree(int x,int y, int str)
  {
  // Wenn ein Controller bekannt ist: Kontakt aller Objekte vor dem Treffer merken
  ...
  if (!inherited(x,y,str, ...)) return false;
  // Für alle Objekte, die danach weniger Kontaktpunkte haben, Controller/Killer setzen
  for (var i=0; i<num; ++i)
    if (GetContact(objs[i], -1, -1) < contacts[i])
      if (GetAlive(objs[i]))
        objs[i]->Schedule(Format("SetKiller(%d)", ctrl), 2, 1); // Killer verzögert setzen, weil Jump-Aktivität ggf. verspätet kommt
      else
