...
func CreateFurniture()
{
  ...
  CreateObject(TBRL, 440, 710, -1)->AutoRespawn();
...
func CreateEquipment()
{
  ...
  var tmp = CreateObject (ABOX, 370, 683, -1);
  tmp->AutoRespawn();
  tmp->Set(AMOC);
  tmp->SetGraphics("Normal");
  var tmp = CreateObject (ABOX, 1255, 190, -1);
  tmp->AutoRespawn();
  tmp->Set(AMOC);
  tmp->SetGraphics("Normal");
  var tmp = CreateObject (ABOX, 1285, 840, -1);
  tmp->AutoRespawn();
  tmp->Set(AMOC);
  tmp->SetGraphics("Normal");

  //Projektilmunition
  PlaceSpawnpoint(AMOC, 1490, 550);

  //Raketen
  PlaceSpawnpoint(MIAP, 1635, 175);
  ...
  //MP7
  PlaceSpawnpoint(MP7R, 365, 495);

  //Pumpgun
  PlaceSpawnpoint(PPGN, 1085, 140);

  //P29
  PlaceSpawnpoint(P29W, 1225, 290);

  //M249
  PlaceSpawnpoint(M249, 840, 330);

  //Predator
  PlaceSpawnpoint(PDTW, 1525, 800);
  ...
  if(FindObject(IGIB))
  {
    ...
    for(var spwn in FindObjects(Find_ID(SPNP)))
      if(Contents(0, spwn)->~IsWeapon())
        RemoveObject(spwn);
    //Keine CMC Waffen-Spawns.
    for(var spwn in FindObjects(Find_ID(SPNP)))
      if(Contents(0, spwn)->~IsWeapon2())
        RemoveObject(spwn);
    ...
    //Keine Kisten mit Waffen.
    for(var ammobox in FindObjects(Find_ID(ABOX)))
      if(ammobox->GetSpawnID())
        if(ammobox->GetSpawnID()->~IsWeapon())
          RemoveObject(ammobox);
    //Keine Kisten mit CMC Waffen.
    for(var ammobox in FindObjects(Find_ID(ABOX)))
      if(ammobox->GetSpawnID())
        if(ammobox->GetSpawnID()->~IsWeapon2())
          RemoveObject(ammobox);
    //Clonk clean?
